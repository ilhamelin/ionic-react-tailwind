...
import { getProductFromFirestore } from "../../../firebase/firebase-functions";
import { IonToast } from "@ionic/react";
import { IoTrophyOutline } from "react-icons/io5";
import { auth } from "../../../firebase/firebase-config";
import { FaCircle } from "react-icons/fa";

const ProductosListLosBajones = ({ idProducto }: { idProducto: string }) => {
        ...
        <div className="flex justify-end font-font-family-light">
          <div className="flex-1 flex-col pr-5">
            <div className="leading-5 mb-1">
              <div className="text-[16px] font-semibold">{productData.nombre}</div>
            </div>
            <div className="flex items-center font-light">
              <div className="text-[14px]">CLP {productData.precio}</div>
              <FaCircle className="mx-[4px] text-[2.7px] " />
              <div className="text-[14px] mr-[4px]">{productData.popularidad}</div>
              <span className="text-[14px]">({productData.cantidadReview})</span>
            </div>
...
export default ProductosListLosBajones;
