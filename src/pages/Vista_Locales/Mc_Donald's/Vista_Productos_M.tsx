...
import { StoreData } from "../../../firebase/firebase-functions";
import ProductosSliderMcDonalds from "./ProductoSliderMcDonalds";
import ProductosListMcDonalds from "./ProductoListMcDonalds";
import ProductosList2x1McDonalds from "./ProductoList2x1McDonalds";
...
const Vista_Productos_M: React.FC = () => {
  ...
  useEffect(() => {
    const fetchStoreData = async () => {
      try {
        const data = await getStoreFromFirestoreVista("4"); // Replace "1" with actual store ID
        if (data) {
  ...
  useEffect(() => {
    const fetchFavorites = async () => {
      if (userId && storeData) {
        ...
        setIsFavorite(favoriteStores.includes("4")); // Replace "1" with actual store ID
      }
    };
  ...
  const handleToggleFavorite = async () => {
    if (isFavorite) {
      await removeFavoriteFromFirestore(userId!, "4"); // Replace "1" with actual store ID
      ...
      await addFavoriteToFirestore(userId!, "4"); // Replace "1" with actual store ID
      setToastAnimation("toast-slide-in");
  ...
  const productosIds2x1 = new Set(["52", "54"]);
          ...
          <div className="flex flex-col x:mt-5 l:mt-5 g:mt-5">
            ...
            <div className="mt-3 mx-4">
              {[...productosIds2x1].map((idProducto) => (
                <div key={idProducto}>
                  <ProductosList2x1McDonalds idProducto={idProducto} />
                </div>
              ))}
            </div>
          </div>
          <div className="flex flex-col x:mt-5 l:mt-4 g:mt-4">
            ...
            <div className="mt-3 mx-4">
              {[...productosIdsSeleccionadoParaTi].map((idProducto) => (
                <div key={idProducto}>
                  <ProductosListMcDonalds idProducto={idProducto} />
                </div>
              ))}
            </div>
          </div>
